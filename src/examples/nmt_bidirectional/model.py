...
def define_nmt(hidden_size, batch_size, en_timesteps, en_vsize, fr_timesteps, fr_vsize):
    ...
    encoder_gru = tf.keras.layers.Bidirectional(
        tf.keras.layers.GRU(hidden_size, return_sequences=True, return_state=True, name='encoder_gru'),
        name='bidirectional_encoder'
    )
    encoder_out, encoder_fwd_state, encoder_back_state = encoder_gru(encoder_inputs)

    # Set up the decoder GRU, using `encoder_states` as initial state.
    decoder_gru = tf.keras.layers.GRU(hidden_size*2, return_sequences=True, return_state=True, name='decoder_gru')
    decoder_out, decoder_state = decoder_gru(
        decoder_inputs, initial_state=tf.keras.layers.Concatenate(axis=-1)([encoder_fwd_state, encoder_back_state])
    )
    ...
    encoder_model = tf.keras.models.Model(inputs=encoder_inf_inputs, outputs=[encoder_inf_out, encoder_inf_fwd_state, encoder_inf_back_state])

    """ Decoder (Inference) model """
    decoder_inf_inputs = tf.keras.layers.Input(batch_shape=(batch_size, 1, fr_vsize), name='decoder_word_inputs')
    encoder_inf_states = tf.keras.layers.Input(batch_shape=(batch_size, en_timesteps, 2*hidden_size), name='encoder_inf_states')
    decoder_init_state = tf.keras.layers.Input(batch_shape=(batch_size, 2*hidden_size), name='decoder_init')
